...
const SplitPane = (props: PropsWithChildren<SplitPaneProps>) => {
  ...
  const boundSplit =
    splitInfo.split < 20 ? 20 : splitInfo.split > 80 ? 80 : splitInfo.split;
  ...
  return (
    <div
      className={`split-pane ${dragging && "dragging"} ${isVertical && "vertical"}`}
      ref={containerRef}
    ...
    >
      <div
        className="left"
        style={{ [!isVertical ? "width" : "height"]: `${boundSplit}%` }}
      >
        {leftNode}
      ...
      <div
        className="right"
        style={{ [!isVertical ? "width" : "height"]: `${100 - boundSplit}%` }}
      >
        {dragging && (
          <div className="view-size">
            {viewWidth}px x {viewHeight}px
          </div>
        )}
        {rightNode}
      </div>
