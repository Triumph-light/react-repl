...
import type { File, ReplStore } from "../component/repl/storeContext";
          ...
          visitor: {
            ExportAllDeclaration(path: NodePath<ExportAllDeclaration>) {
              ...
              s.overwrite(
                node.start!,
                node.end!,
                `for (const key in ${modulesKey}["${modulePath}"]) {
                            if (key !== 'default') {
            ...
            ExportDefaultDeclaration(path: NodePath<ExportDefaultDeclaration>) {
              const { node } = path;
              s.append(
                `${moduleKey}.default = ${(node.declaration as Identifier).name}\n`,
              );
            ...
            ExportNamedDeclaration(path: NodePath<ExportNamedDeclaration>) {
              ...
              // export { foo, bar } from './foo'
                ...
                const modulePath = node.source.value.replace(/\.\//, "");
                const expandNames = node.specifiers.map(
                  (specifier) => (specifier as ExportSpecifier).local.name,
                );
                if (expandNames && expandNames.length !== 0)
                  s.overwrite(
                    node.start!,
                    node.end!,
                    `const { ${expandNames.join(",")} } =  ${modulesKey}["${modulePath}"]`,
                  );
            ...
            ImportDeclaration(path: NodePath<ImportDeclaration>) {
              const { node } = path;
              const importDefault = node.specifiers.find(
              ...
              if (node.source.value.startsWith("./")) {
                ...
                if (importDefault) {
                  ...
                  s.overwrite(
                    node.start!,
                    node.end!,
                    `const ${moduleName} = ${modulesKey}["${modulePath}"].default\n`,
                  );
                } else {
                  const nameSpaceObj = node.specifiers.find(
                    (specifier) =>
                  ...
                  if (nameSpaceObj) {
                    ...
                    s.overwrite(
                      node.start!,
                      node.end!,
                      `const ${nameSpaceObj.local.name} = ${modulesKey}["${modulePath}"]`,
                    );
                  } else {
                    // import { foo } from 'xx'
                    const expandNames = node.specifiers?.map(
